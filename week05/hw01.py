...
#select = input("請從以上四個縣市選擇一個：")
select = '金門縣'
selectTable = dicfile[dicfile['縣市'] == select]
print(select + '的門市')
for i in range(selectTable.shape[0]):
    print("%2d) %-*s %2s"%(i+1, 4, str(selectTable.iat[i, 1]) + "店 ",  selectTable.iat[i, 2]))
